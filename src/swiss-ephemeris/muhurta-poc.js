/**
 * Swiss Ephemeris Muhurta Proof of Concept (JavaScript version)
 * 
 * This file demonstrates basic usage of the sweph library for astrological calculations
 * relevant to muhurat determination, implemented in plain JavaScript to avoid ts-node issues.
 */
...
function calculateTithi(sunLongitude, moonLongitude) {
  ...
  // Determine paksha (bright half or dark half)
  const paksha = tithi <= 15 ? 'Shukla' : 'Krishna';
  
  // Get tithi name (1-15 within each paksha)
...
// Test function to run the proof of concept
function runTest() {
  console.log('\n==== Starting Swiss Ephemeris Muhurta PoC Test ====');
  ...
  try {
    ...
    // Display muhurta results
    if (muhurta.success) {
      console.log('\nMuhurta calculation successful!');
      console.log(`Date: ${muhurta.date}`);
      console.log(`Sunrise: ${muhurta.sunrise.hour.toFixed(2)} (${Math.floor(muhurta.sunrise.hour)}:${Math.floor((muhurta.sunrise.hour % 1) * 60).toString().padStart(2, '0')})`);
      console.log(`Sunset: ${muhurta.sunset.hour.toFixed(2)} (${Math.floor(muhurta.sunset.hour)}:${Math.floor((muhurta.sunset.hour % 1) * 60).toString().padStart(2, '0')})`);
      ...
      muhurta.muhurtas.forEach(m => {
        ...
        console.log(`   Tithi: ${m.tithi}, Yoga: ${m.yoga}, Nakshatra: ${m.nakshatra}`);
      });
    } else {
      console.error('\nMuhurta calculation failed:', muhurta.message);
    }
  } catch (error) {
    console.error('\nError in test:', error);
  } finally {
    // Close Swiss Ephemeris
    console.log('\nClosing Swiss Ephemeris...');
    sweph.close();
  }
