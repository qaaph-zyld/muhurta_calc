...
function calculateSunriseSunset(year, month, day, latitude, longitude) {
  try {
    // Calculate Julian day for the given date
    const julianDay = sweph.julday(year, month, day, 0, SE_CONSTANTS.SE_GREG_CAL);
    
    // Use a more accurate approximation based on latitude and time of year
    // This is a fallback method that doesn't rely on rise_trans
    ...
    const decl = 23.45 * Math.sin(Math.PI / 180 * 360 * (dayOfYear - 81) / 365);
    
    // Calculate sunrise and sunset hour angles
    const hourAngle = Math.acos(-Math.tan(latitude * Math.PI / 180) * Math.tan(decl * Math.PI / 180)) * 180 / Math.PI;
    ...
    const sunriseJD = julianDay + (sunriseHour / 24);
    const sunsetJD = julianDay + (sunsetHour / 24);
...
function calculateMuhurta(year, month, day, latitude, longitude) {
  try {
    ...
    const { sunriseJD, sunsetJD } = calculateSunriseSunset(year, month, day, latitude, longitude);
